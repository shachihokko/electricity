...
def make_se_list(args):
  ...
  #データの長さの調整
  if n!=0:
    mx_x_iip_tmp = mx_x_iip[0:-n,:]
    mx_y_tmp = mx_y[0:-n,:]
    mx_elec_tmp = mx_elec[0:-n,:]
    mx_const_tmp = mx_const[0:-n,:]
  elif n==0:
  ...
  for i in tqdm(range(num_batch), desc=info, position=n+1):
    coeff = coef_matrix[:,i]
    SE = SE + [float(eval_SE(mx_y_tmp, np.concatenate([mx_const_tmp, mx_x_iip_tmp,np.dot(mx_elec_tmp[:, j], coeff).reshape([row,1])],1))) for j in vc_list]
  #バッチ毎の結果のまとめ
  return SE
...
if __name__ == "__main__":
  ...
  #並列化時の引数の作成
  wrap_multi_args =[(i, num_batch, mx_x_iip, mx_y, mx_elec, mx_const, coef_matrix, vc_list) for i in range(RMSE_loop)]

  #並列化処理
  with Pool(processes=os.cpu_count(), initializer=tqdm.set_lock, initargs=(RLock(),)) as p:
    result = p.map(wrapper_make_se_list, wrap_multi_args)

  # tqdm終了後のカーソル位置を最下部に持ってくる
  print("\n" * os.cpu_count())

  #時系列が逆方向になってるので反転させる（新しいのを下にする）
  result = np.flip(np.array(result), axis=0)
  rmse = np.sqrt(np.mean(result, axis=0))
  output = pd.DataFrame()
  output["RMSE"]=rmse
  output.to_csv(dir+"RMSE_"+str(num_vars)+".csv.bz2", compression='bz2', encoding="cp932")
